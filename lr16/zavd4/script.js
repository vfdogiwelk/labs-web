...
for (let i = 0; i < products.length; i++) {
    ...
    discount.textContent = "Знижка: " + product.discountPercentage + "%";
    ...
    let stars = "";
    for (let j = 0; j < 5; j++) {
        if (j < Math.round(product.rating)) {
            stars += "★";
        } else {
            stars += "☆";
        }
    ...
    brand.className = "card-info";
    brand.textContent = "Бренд: " + product.brand;
    body.appendChild(brand);

    let category = document.createElement("div");
    category.className = "card-info";
    ...
    stock.className = "card-info";
    stock.textContent = "В наявності: " + product.stock;
    ...
    cartBtn.addEventListener("click", function () {
        console.log("Додано товар з id: " + this.getAttribute("data-id"));
    });
